...
contract VRFCoordinator is VRF, VRFRequestIDBase {
  ...
  event TokenTransferred(address _sender, uint256 _fee, bytes _data);
  /**
    ...
    onlyLINK
  {
    ...
    emit TokenTransferred(_sender, _fee, _data);
    ...
    sufficientLINK(_feePaid, _keyHash)
  {
    ...
    // brian
    // as we do not have orcale, just call callBackWithRandomness
    // Callback memory callback =  callbacks[requestId];
    uint randomness = 0x1234 ; // set the magic number for the test
    // delete callback from the mapping
